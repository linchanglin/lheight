...
Page({
    data: {
        // 测试用
        test_loves: [
            {id: 1, content: '北京大学（Peking University）简称“北大”，诞生于1898年，初名京师大学堂，是中国近代第一所国立大学，也是第一个以“大学”之名创办的学校，其成立标志着中国近代高等教育的开端。北大是中国近代以来唯一以国家最高学府身份创立的学校，最初也是国家最高教育行政机关，行使教育部职能，统管全国教育。北大催生了中国最早的现代学制，开创了中国最早的文科、理科、社科、农科、医科等大学学科，是近代以来中国高等教育的奠基者。'},
            {id: 2, content: '清华大学（Tsinghua University），简称“清华”，由中华人民共和国教育部直属，中央直管副部级建制，位列“211工程”、“985工程”，入选“珠峰计划”、“2011计划”、“111计划”、“卓越工程师教育培养计划”、“卓越法律人才教育培养计划”、“卓越医生教育培养计划”，为九校联盟、东亚研究型大学协会、环太平洋大学联盟、亚洲大学联盟[1]  、清华大学—剑桥大学—麻省理工学院低碳能源大学联盟成员。'},
        ],
    ...
    get_unreadLoveNums: function () {
        ...
        let wesecret = wx.getStorageSync('wesecret');
        if (wesecret) {
            url = `https://collhome.com/beijing/apis/unreadLoveNums?love_id=${love_id}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/beijing/apis/unreadLoveNums?love_id=${love_id}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
    ...
    load_loves: function (parameter) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${page}&search=${search}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${page}&search=${search}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
