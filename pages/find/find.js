...
Page({
    data: {
        // 测试用
        test_loves: [
            { id: 1, content: '复旦大学（Fudan University），简称“复旦”，位于中国上海，由中华人民共和国教育部直属，中央直管副部级建制，位列“211工程”、“985工程”，入选“珠峰计划”、“111计划”、“2011计划”、“卓越医生教育培养计划”，为“九校联盟”成员、中国大学校长联谊会成员、东亚研究型大学协会成员、环太平洋大学协会成员、21世纪大学协会成员，是一所综合性研究型的全国重点大学。' },
        ],
    ...
    load_imageLoves: function (parameter) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/shanghai/apis/loves?type=${type}&page=${image_page}&search=${search}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/shanghai/apis/loves?type=${type}&page=${image_page}&search=${search}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
    ...
    load_videoLoves: function (parameter) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/shanghai/apis/loves?type=${type}&page=${video_page}&search=${search}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/shanghai/apis/loves?type=${type}&page=${video_page}&search=${search}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
