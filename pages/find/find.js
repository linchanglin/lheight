...
Page({
    data: {
        // 测试用
        test_loves: [
            {id: 1, content: '清华大学诞生于1911年，依托美国退还的部分“庚子赔款”建立，因北京西北郊的清华园而得名，初称“清华学堂”，是清政府设立的留美预备学校，翌年更名为“清华学校”；为尝试人才的本地培养，1925年设立大学部，1928年更名为“国立清华大学”；1937年抗日战争爆发后，学校南迁长沙，与北京大学、南开大学联合组建“国立长沙临时大学”；1938年迁至昆明，改名为“国立西南联合大学”；1946年迁回清华园，1952年成为一所多科性工业大学；改革开放以来，学校先后恢复或新建了理科、经济、管理和文科类学科，并成立了研究生院和继续教育学院'},
        ],
    ...
    load_refresh_loves: function (need_refresh_love_id) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/beijing/apis/loves/${need_refresh_love_id}?wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/beijing/apis/loves/${need_refresh_love_id}?wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
    ...
    load_hotLoves: function (parameter) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${hot_page}&search=${search}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${hot_page}&search=${search}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
    ...
    load_imageLoves: function (parameter) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${image_page}&search=${search}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${image_page}&search=${search}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
    ...
    load_videoLoves: function (parameter) {
        ...
        let url;
        if (wesecret) {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${video_page}&search=${search}&wesecret=${wesecret}`
        } else {
            url = `https://collhome.com/beijing/apis/loves?type=${type}&page=${video_page}&search=${search}&wesecret=`
        }
        wx.request({
            url: url,
            success: function (res) {
